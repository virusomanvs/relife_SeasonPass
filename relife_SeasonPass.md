...
| `changeSeasonPlayerAttempt`          | `int`  |  Сколько попыток дается игроку в сезоне, чтобы сменить сезонные задания. Задания будут выбраны случайно из доступных свободных сезонных заданий.|
...
| `savePlayerDataBin`          | `bool`  |При включении конфиги будут сохраняться и загружаться в бинарном виде.|
| `savePlayerDataSQL`          | `bool`  |При включении конфиги будут сохраняться и загружаться из сторонней БД от KGB TOOL. Virusoman_BP добавьте в config|
| `savePlayerDataHUB`          | `bool`  |При включении конфиги будут сохраняться и загружаться из сторонней БД RELIFEHUB|
| `resaveConfigAfterStart`          | `bool`  |При включении конфиги будут пересохраняться после старта сервера. (Можно запустить 1 раз с этим параметром после обновления, чтобы в конфиги добавились новые добавленные значения)|
...
| `enableSpawnTransportInZone`          | `bool`  |Если включить данную настройку, то транспорт можно будет получить как награду, только в определенном месте.|
| `spawnTransportZone`          | `vector`  |Точка где можно будет получить транспорт.|
| `spawnTransportZoneRadius`          | `int`  |Радиус от точки для спавна транспорта.|
...
ВНИМАНИЕ! После параметра     "seasonPassList": [], и включая его, ничего добавлять не надо, все загрзится из сторонних конфигов.
