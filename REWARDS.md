...
| Поле              | Тип        |  Описание |
|-------------------|------------|----------|
| `rewardUniqueID`  | `integer`  | Уникальный идентификатор награды, который не может повторяться! Используйте знак - для указания случайно награды из конфига [config_RandomItemsList.json](https://github.com/virusomanvs/relife_SeasonPass/blob/main/config_RandomItemsList.md)|
